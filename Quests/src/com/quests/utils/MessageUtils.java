...
public class MessageUtils {
	...
	public static void inform(Player player, String s) {
		player.sendMessage(Quests.getInstance().configManager.messages.prefix + MessageUtils.translateAlternateColorCodes(s));
	}
	...
	public static void sendHelp(Player player) {
		 Quests.getInstance().CommandManager.getCommands().forEach(i -> {
			 MessageUtils.sendRawMessage(player,Quests.getInstance().configManager.messages.prefix + " " + "&6• &c" + i.name() + "&7 : &b" + i.info());
		 });
	}
}
