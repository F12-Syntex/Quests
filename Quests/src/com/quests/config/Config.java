	...
	public File backup() {
		...
		if(backups.mkdirs() || backups.exists()) {
			...
			if(!this.getConfig().exists()) return null;
				...
				if(success) {
					Quests.Log("&a" + this.name + ".yml has been backed up!");	
				}else {
					Quests.Log("&cCouldnt backup!");
				}
			
				return tempConfig;
			
		}else if(!backups.exists()) {
			Quests.Log("&cCouldnt create backup folder!");
		}
